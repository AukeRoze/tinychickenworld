...
- [x] **Orchestrator-state-machine-integratietest — GEBOUWD (2026-06-12).** `PipelineOrchestratorStateMachineTest` (pure Mockito + JUnit 5, geen Spring-context/DB; map-backed repo-mock, `self`-veld via ReflectionTestUtils → pipeline draait synchroon). 8 tests: happy path met gepinde statusreeks + client-volgorde; alle 5 productie-gates pauzeren/approven één hop per keer; QA-publish-gate houdt unpublishable master vast tot human override; stage-falen → FAILED zonder downstream-calls; retry hervat op ASSEMBLING zonder asset-regeneratie (@TempDir-bestanden); resumeAfterRestart hergebruikt bestaande assets; approve/retry-guards. Alle 25 constructor-velden + client-signatures letterlijk tegen de bron geverifieerd (incl. 6-args thumbnail-overload, regel 1494). Veo-pad/song-mode/Auto-Fix bewust niet gedekt (eigen flows, later op dezelfde steiger). Let op: ReflectionTestUtils op private veldnamen (`qaGateEnabled` e.d.) — hernoemen breekt runtime, niet compile-time. **Ongecompileerd → `mvn -pl services/orchestrator test` bij de rebuild. Dit ontgrendelt @Version op VideoJob.**
- [ ] **PipelineOrchestrator → stage-services** splitsen (incrementeel; 2.264 regels nu).
- [x] **GET-mutaties vervangen — GEBOUWD (2026-06-12).** De GET-twins van approve/reject/lock-all (ReviewController) en retry/reassemble/autofix (VideoController) zijn verwijderd — de UI gebruikte al overal POST (job-page.js geverifieerd), de GET's waren er alleen voor de inmiddels retired reviewmails. Voor toekomstige mails: nieuwe `ReviewTokenService` (HMAC-SHA256, `REVIEW_TOKEN_SECRET`, TTL 7d, constant-time verify) + `GET /api/v1/review/confirm?token=` (alleen bevestigingspagina) → menselijke klik → `POST /api/v1/review/execute` (pas dán de mutatie). Unit-test `ReviewTokenServiceTest` (rond-trip, expiry, tampering). Kanttekening: tokens zijn stateless (geen replay-store) — acceptabel want de acties zijn status-geguard. **Ongecompileerd → build.bat.**
...
- [x] **Cast bewerkbaar vanuit de UI (v1) — GEBOUWD.** Cast-pagina heeft per character **✎ Bewerken**: naam/rol/`dna.accessory`/`dna.tic` aanpassen + een **referentieplaatje uploaden** (`bible/refs/{id}.png`). Schrijft via nieuwe `BibleEditor` (comment-behoudende in-place YAML-edit, maakt `channel.yml.bak` + valideert en revert bij ongeldige uitkomst) en `BrandController` (`POST /api/v1/brand/cast/{id}`, `POST /cast/{id}/image`). Orchestrator-bible-mount nu read-write; multipart-limiet op 15MB. **Geen training nodig** want `IMAGE_PROVIDER=gemini` (reference-conditioned). Caveat: image-/script-/voice-service cachen de bible bij opstart → na een edit **orchestrator + image-service herstarten** voor nieuwe renders (Cast-pagina zelf toont direct). **Uitgebreid:** ook `description`, `personality` (gevouwen blokken) en `catchphrases` (opener/closer-lijsten) zijn nu bewerkbaar (comment-behoudende blok/lijst-editing in `BibleEditor`, zelfde backup+validate+revert-net). De Cast-pagina toont nu de eigenaardigheden: accessoire, tic, signatuurgeluid, **persoonlijkheid**, catchphrases en uiterlijk. `personality`+`catchphrases` voeden de script-prompt (PromptBuilder), `tic` de Veo-prompt — dus edits sturen echt de output. **Open vervolg:** ~~een "reload bible"-endpoint i.p.v. restart~~ → **GEBOUWD (2026-06-12)**: `POST /api/v1/bible/reload` op script-/voice-/image-/thumbnail-/videogen-service (BibleLoader.load() her-aanroepbaar; thumbnail-loader cleart nu z'n cast-collecties), orchestrator `BibleReloadService` (leegt VeoPromptCompiler-caches + fan-out, best-effort) + `POST /api/v1/brand/bible/reload`, Cast-edits (`updateCharacter`/`uploadReference`) triggeren het automatisch, en de Brand-pagina heeft een 🔄 Reload-bible-knop. De herstart-caveat vervalt. Assembly hot-reloadt al zelf; upload leest geen bible. Resteert: optioneel een referentieplaatje AI-genereren i.p.v. uploaden. Training-vanuit-UI (Replicate LoRA) alleen nodig als je naar `replicate` switcht — groot apart traject, niet aanbevolen.

- [x] **Intro/outro preview in het dashboard — BLEEK AL GEBOUWD** (vastgesteld 2026-06-12): `IntroController`/`OutroController` serveren `GET /api/v1/{intro,outro}/current.mp4` en brand.html heeft `<video>`-spelers naast de rebuild-knoppen, incl. cache-busted herladen na een rebuild-poll. Backlog-item was verouderd.
...
- [x] **Banner-canon checken — GECHECKT (2026-06-12), geen fout.** De groene sjaal in de banner zit op **Bo** (ronde bril — canon: bril + groene sjaal), niet op Mo. Mo (blauwgrijs, hoge rode kam, links) draagt rood; Pip (midden) strohoed + rode bandana. Banner is canon-conform. Nuance: Mo's halsdoek oogt op bannerresolutie dunner dan "chunky knitted", maar kleur klopt en de banner is de canon-bron.
...
- [x] **Serie-mythologie — GEBOUWD (2026-06-12).** Nieuw `seriesMythology`-blok in `bible/channel.yml`: openingsritueel ("tap-tap-tap hello" van Pip, in de eerste hook-scène, ≤4s) + één running gag per personage (Pip hat-shelf, Mo stretched count "two-and-a-HALF", Bo mis-hear-rijm, Duckling mistimed KWAK). Script-service: `SeriesMythology`-record + `BibleLoader.parseSeriesMythology` + `PromptBuilder.renderSeriesMythology` (eigen sectie, ná EPISODE STRUCTURE). Dosering afgedwongen in de prompt: max 1 gag-moment per personage, max 2 totaal, verbale gag telt mee in het tic-budget. Leeg blok = sectie verdwijnt (oudere bibles ok). Rituelen bewerken = alleen bible-edit + script-service-herstart. **Ongecompileerd → build.bat.**
...
- [x] **Slimmere auto-Short — GEBOUWD (2026-06-12).** `ShortsBuilder.energeticStart` koos al loudness-gebaseerd, maar op het **luidste losse sample** — één gasp/sting kon het venster kapen midden in een rustig stuk. Nu: short-term loudness per seconde gebucket en een **sliding window van de volle cliplengte** over het gemiddelde geschoven → het segment met de hoogste **aanhoudende** energie wint (lach+viering-run i.p.v. één piek). Seconden zonder meting tellen als stilte (-70 LUFS), dus pauzes/outro-fade drukken de score. Zelfde fallback (~55%) bij scan-falen. **Ongecompileerd → build.bat.**
...
- [x] **Cast-thumbnail breder triggeren — GEBOUWD (2026-06-12).** Niet langer alleen tekst-heuristiek (namen/group-cues in titel/topic): de orchestrator stuurt nu **ground truth** mee — `presentCast()` = bible-ids die in ≥35% van de scènes (min. 2) zitten — via een nieuwe 8-args `thumbnailClient.generate`-overload → `GenerateThumbnailRequest.castPresent`. Thumbnail-service: ≥2 ids = groeps-thumbnail (ook bij anonieme titels als "The Big Puddle Mystery"), en `anchorBases` rendert alléén de aanwezige castleden (afwezig eendje sluipt nooit de thumbnail in). Tekst-heuristiek blijft fallback. State-machine-test bijgewerkt op de nieuwe overload. **Ongecompileerd → build.bat.**
