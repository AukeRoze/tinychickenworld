...
- [ ] **Pixar Stories** (specs in `analyse/PIXAR-QUALITY-AUDIT.md`): C volledige shot/reverse-dialoogdekking (kost clips; wachten op retentie-data).
- [~] **Story G multi-angle — NU VIA UI ACTIVEERBAAR (2026-06-12).** Nieuwe "📐 Extra hoek"-knop op de Cast-pagina: genereert hoeken (zijaanzicht / driekwart-achter; driekwart-vóór bewust niet — wimper-drift) **geconditioneerd op de gekozen primaire ref** (`characters:[id]` → provider ankert op `refs/{id}.png`), approve → additief naar `refs/{id}/{hoek}.png` (cap 3, provider-limiet). (user) Per kip 1-2 hoeken genereren ná het redesign — ~€0,10-0,20 per kip.
...
- [x] 🎨 **Overlay-logo regenereren met de rebranded cast** ("zelfde bord, nieuwe kippen"): nieuw additief `styleAnchors`-veld in de image-service (design-ref gaat als referentiebeeld mee: "copy its exact composition, board, lettering — only the characters change"; `logo-source.png` als design-bron), kandidaten op puur-magenta achtergrond, bij approve automatische chroma-key + rand-decontaminatie (de-halo) naar transparant → zelfde validatie/backup-route als de upload.
- [x] 📈 **Poll-analytics-knop in de topbar** (elke pagina, rechtsboven): `POST /api/v1/analytics/poll` met uitleg-tooltip + laatste-poll-tijd als geheugensteun. NB de poller draait sowieso automatisch elke 6 uur — de knop is "ik wil het nú zien".
- [x] **Pip petite-pass** (gebruikerswens): DNA aangescherpt — Pip nu de kléínste (petite, fijn, extra groot hoofd + grote ogen = babyproporties), nieuwe orde Mo=grootste/breedste · Bo=dunste · Pip=kleinste rondje; Mo/Bo-scaleAnchors consistent meegetrokken. Uitrollen = 🔄 reload + 🎨 nieuwe Pip-ref kiezen.
- [x] **Logo-regen fix + plan B**: REPAINT-instructie aangescherpt aan beide kanten (oude personages in het design-beeld = VEROUDERD, niet kopiëren; cast-refs winnen) + UI-vinkje "oude logo als design-referentie (uit = vrij genereren)" → `useDesignRef:false` = tekstueel beschreven bord (TINY/CHICKEN goud-geel, WORLD blauw, gouden ei) zonder oud beeld — gegarandeerd de nieuwe cast.
- [x] **Chroma-key adaptief** (fix op live 400): meet nu de werkelijke randkleur (buitenste 2px, egaliteits-check) en keyt dáárop i.p.v. hard op #FF00FF — roze-drift/wit/crème werkt nu ook; magenta-fallback + >97%-wis-vangrail; 400-melding bevat voortaan de gemeten randkleur als diagnose.
- [x] **🔁 Re-render beelden (nieuwe cast)** — knop op de job-pagina (review/terminal-statussen): zelfde script + stemmen, álle beelden/Veo-clips/end-stills vers met de huidige refs; wist episode-anchors (oude-cast-canon), thumbnail, scene-locks én het reuse-images-veld (edge case: zou anders oude beelden terugkopiëren); hervat op de assets-stage, QC/gates draaien normaal mee. + 2 unit-tests. Dé knop om bestaande afleveringen naar de rebranded look te tillen.
- [x] **Correctie-hint bij regen** (gebruikersobservatie: regen was blind, geen feedback de prompt in — geverifieerd correct). Additief `correctionHint`-veld door de hele regen-keten (`regenScene`/`regenerateSceneImage` → image-payload → alle 3 providers via `PromptComposer.withCorrection`: "IMPORTANT CORRECTION — fix specifically: {hint}"). Handmatige "↻ Regen" opent nu een invulvenster; QC-reroll en Auto-Fix geven hun issue-/Critic-tekst voortaan mee i.p.v. alleen loggen.
- [x] **Veo Lite-404 + cost-rate-bindingbug (2026-06-13)**: standaard-scènes routeten via de cost-cap-downshift naar het hardcoded `veo-3.1-lite-generate-preview` → 404 (model bestaat niet in dit GCP-project). Fix: `ModelRouter.cheapest()` + de `veo3_1_lite`-alias resolven nu naar `veo-3.1-fast-generate-001` (bestaand GA-model); ModelRouterTest-assertie meegewijzigd. DIEPERE oorzaak: de cost-rates stonden wél in application.yml maar Spring kon ze niet binden — de model-ids bevatten PUNTEN (3.1) die Spring's relaxed binding als property-pad las → rate nooit in de Map → eindeloze "No rate €0.5/s" → te-vroege downshift. Fix: alle veo-rate-sleutels in **bracket-notatie** `"[veo-3.1-...]"` (zoals de Seedance-keys al deden). Dit was de verborgen wortel onder uren symptomen.
- [x] **Veo "Alle scènes → Veo"-knop** + cost-cap-rebuild + **quota-classificatie-fix**: live bleek dat bij een burst Veo-calls 84% faalde als FALLBACK(OTHER) — root cause: Vertex weigert met `RESOURCE_EXHAUSTED` (quota), maar `classify()` herkende alleen "429"/"quota" → viel in OTHER → direct Ken Burns zonder retry. Fix: RESOURCE_EXHAUSTED/code 8/exhausted → `Kind.QUOTA`; echte exponentiële backoff-retry (5/15/45s, `VEO_QUOTA_MAX_RETRIES`/`VEO_QUOTA_BASE_BACKOFF_MS`) op hetzelfde model vóór de Ken Burns-fallback; `VEO_MAX_PARALLEL` default 3→2 om Vertex minder te overbelasten. Plus eerder: Veo image+refs-conflict (mutually exclusive) deterministisch opgelost, assembly-timeout → async assemble. Kanttekening: blijft op message-matching leunen (SDK-tekst niet live geverifieerd).
- [x] **Eindframe-interpolatie voor alle Veo-scènes** (anti-drift, `VEO_END_FRAME_ALL` default aan): elke Veo-scène krijgt nu een eindbeeld (default "moment later, beweging voltooid, identiek personage") zodat Veo tussen twee vastgezette frames interpoleert i.p.v. de beweging vrij te verzinnen. Chaining uit bij deze modus (expliciet eindframe wint). ~€0,02 extra Gemini-still per scène, geen Veo-meerkost. Kanttekening: dempt geleidelijke drift, niet de wilde hallucinatie (vikinghelmen) op een verse rebrand — A/B'en.
- [x] **QC-review: afgekeurde Veo-clips bewaren + bekijken + overrulen**: clip-QC verwijderde afgekeurde clips (gebruiker kon de QC niet controleren). Nu → `clip.rejected.mp4` bewaard + `qcRejectReason` op de scène; job-pagina toont "⚠ QC afgekeurd"-box + reden, "▶ Bekijk afgekeurde clip" en "✓ Toch gebruiken (override QC)" (promoveert terug + hermonteert, geen Veo-kosten). Werkt vanaf de volgende run (al-verwijderde clips zijn weg).
- [x] **Uitleg-box scène-acties**: uitklapbare ⓘ-box bovenaan de scènelijst die alle per-scène-knoppen verklaart, met nadruk op 🔒 Lock (= beschermt tegen de automatische QC/Auto-Fix, niet tegen handmatige edits). Open/dicht-stand in localStorage. Eénmalig, voor iedereen zichtbaar.
- [x] **Regen-laadindicatie**: bij een scène-regen/edit/eindbeeld bleef het oude beeld staan zonder feedback (Gemini duurt 10-30s). Nu dimt het beeld en draait er een spinner met "Nieuw beeld genereren…" over het frame zolang de (synchrone) regen-POST loopt; klaart op zodra het nieuwe beeld er is. Pure frontend (CSS-spinner + `setSceneBusy`-helper).
- [x] **Typen onmogelijk-bug gefixt**: de 5s-poll op de job-pagina herbouwde scène/review-kaarten (replaceChildren) en blies elk open tekstveld + getypte tekst weg. `load()` pauzeert de verversing nu zolang een tekstveld/textarea/select/contenteditable focus heeft (checkboxes/knoppen blokkeren niet); hervat automatisch bij blur/submit.
- [x] **QA-hint prefill** in de regen-dialoog: `GET /api/v1/videos/{id}/scenes/{seq}/regen-hint` stelt per scène een voorstel samen (prio: persisted QC-issue > Critic-finding die een aanwezig character noemt > lage Characters/Animation-as), de prompt() wordt ermee voorgevuld — overschrijven/wissen kan. Best-effort, nooit 500. **Bekend gat:** QA-board heeft geen per-as detailtekst voor Characters/Animation (alleen generieke nudge); de echte specificiteit komt uit de QC-issue + Critic-finding.
